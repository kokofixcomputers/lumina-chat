...
export default function SettingsPanel({
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'providers' | 'data' | 'cloudsync' | 'tools' | 'about'>('general');
            ...
            {activeTab === 'general' ? (
              <div className="flex-1 overflow-y-auto p-5 space-y-6 max-w-2xl">
                ...
                <section>
                  ...
                  <div className="form-group">
                    <label className="form-label">SerpAPI API Key</label>
                    <input
                      type="password"
                      value={settings.serpApiKey || ''}
                      onChange={e => onUpdateSettings({ serpApiKey: e.target.value })}
                      className="input text-sm font-mono"
                      placeholder="Enter your SerpAPI key"
                    />
                    <p className="form-help">Required for Google search tool functionality</p>
                  </div>
                </section>
